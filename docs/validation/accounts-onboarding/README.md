...
- Investment accounts recognized by the institution catalog are excluded from the bank tile. Investment detail includes brokerage snapshots and access to manual investment accounts. Unknown prices/rates and load errors are not rendered as zero. Manual balances and snapshots are not silently added together, since they may describe the same portfolio; source-separated details make this explicit. Snapshot values remain separate from the existing net-worth calculation, with a visible note whenever snapshots exist.
